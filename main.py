...
# ==================================================
# FETCH (GÜVENLİ, USER-AGENT, RETRY)
# ==================================================
HEADERS = {
        ...
        try:
            async with session.get(url, params=params, headers=HEADERS, timeout=10) as resp:
                if resp.status == 200:
                    ...
                    # Yetki hatası veya yasal engel -> hemen vazgeç
...
# ==================================================
# EXTERNAL DATA (ana session kullanıyor)
# ==================================================
...
async def get_liquidation_data(session, symbol):
    ...
    try:
        ...
        # Coinglass için ayrı bir oturum açmıyoruz, ana session'ı kullanıyoruz
...
# ==================================================
# MAIN (TEK SESSION, TEMIZ KAPANMA)
# ==================================================
async def main():
    print("🚀 HİBRİT BOT (Sağlam Bağlantı)")
    async with aiohttp.ClientSession() as session:
        # Telegram worker başlat
        worker = asyncio.create_task(telegram_worker(session))
        await send_telegram("✅ BOT ONLINE (User-Agent eklendi)")

        # API testi
        if await fetch_json(session, "/fapi/v1/ping") is not None:
            await send_telegram("🌐 Binance bağlantısı başarılı")
        else:
            await send_telegram("⚠️ Binance bağlantısı başarısız, ama denemeye devam edeceğim.")

        syms = await get_all_symbols(session)
        print(f"{len(syms)} coin")
        sem = asyncio.Semaphore(MAX_CONCURRENT_REQUESTS)

        # Backtest arka planda
