...
# ==================================================
# SINIFLANDIRMA (FLOOR İPTAL EDİLEBİLİR)
# ==================================================
...
# ==================================================
# BACKTEST (FLOOR İPTAL, LOGLU)
# ==================================================
async def run_backtest(session):
    try:
        await send_telegram("📊 BACKTEST BAŞLADI (floor iptal, çok gevşek)")
        total_signals = 0
        wins = 0
        total_pnl_net = 0.0
        ...
        for symbol in test_symbols:
            ...
            for i in range(200, len(klines)-1):
                ...
                best_score = max(long_score, short_score)
                # Floor olmadan, vol_factor 0.3 → weak=1.5
                signal_type = classify_signal(best_score, 0.3, use_floor=False)
                if not signal_type:
                    continue
            ...
            print(f"{symbol}: {symbol_signals} sinyal")
        ...
        msg = (f"📊 BACKTEST SONUCU (floor iptal)\n"
...
# ==================================================
async def main():
    print("🚀 BOT BAŞLATILDI (Backtest floor iptal)")
    async with aiohttp.ClientSession() as session:
        asyncio.create_task(telegram_worker(session))
        await send_telegram("✅ BOT ONLINE (Backtest iyileştirildi)")
